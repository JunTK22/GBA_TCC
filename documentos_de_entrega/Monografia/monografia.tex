...
barramento, canal de DMA e controlador de SDRAM para a ROM de
cartucho, já se encontra descrito e integrado ao \textit{datapath},
...
the bus controller and arbiter, a DMA channel and an SDRAM controller
for the cartridge ROM, is already described and integrated into the
...
acesso direto à memória~\cite{Copetti2019,Vijn_Tonc}.
...
ROM de cartucho (\texttt{PAK\_ROM}) por meio de um controlador
dedicado~\cite{CycloneV}, ficando as demais regiões em memória
\textit{on-chip} (E4). A saída gráfica demanda a geração de um
...
sintetizáveis das regiões internas (BIOS, EWRAM, IWRAM, I/O, Palette e
OAM), o controlador de SDRAM para a ROM de cartucho externa
(\texttt{PAK\_ROM}), o controlador de barramento com decodificação por
...
Pocket~\cite{AnaloguePocket}, produto comercial baseado em FPGA,
reproduz Game Boy, Game Boy Color e Game Boy Advance e demonstra a
viabilidade industrial da abordagem. Dão suporte a todas essas
...
Terasic~\cite{TerasicDE1SoC}, que abriga uma FPGA Cyclone~V
(\mbox{5CSEMA5F31C6}~\cite{CycloneV}). Entre os recursos da placa
relevantes ao projeto, destacam-se os 64~MiB de SDRAM externa de
16~bits, destinada exclusivamente a abrigar a ROM de cartucho
(\texttt{PAK\_ROM}), região que, na fase final do projeto, poderá
...
regiões internas do mapa (BIOS, EWRAM, IWRAM, Palette RAM, VRAM e OAM). A saída VGA,
...
ModelSim/QuestaSim por meio de \textit{scripts} TCL dedicados a cada
cenário de teste. Os \textit{cores} de PLL, de controle de
...
\texttt{dma.v} (canal DMA0, no momento) executa as transferências em
bloco a partir dos registradores de controle providos pelo
\texttt{io\_registers.v}. Os módulos de região (\texttt{bios.v},
\texttt{ewram.v}, \texttt{iwram.v}, \texttt{io\_registers.v},
...
A ROM de cartucho (\texttt{PAK\_ROM}) é a única região mapeada na
SDRAM externa da DE1-SoC, servida por um controlador de SDRAM próprio
...
coincida com um ciclo de \textit{refresh} não seja perdida. Como a
região é escrevível, prevê-se, na fase final do projeto, utilizá-la
para carregar uma ROM a partir de um cartão SD para a SDRAM.
...
simulação RTL no QuestaSim, dirigida por \textit{scripts} TCL
(\texttt{test.tcl} e \texttt{interrupt\_test.tcl}), com inspeção das
formas de onda dos barramentos, dos registradores do
...
localizado em \texttt{documentos\_de\_entrega/Cronograma/}.
% [INDICAR: o cronograma detalhado ainda será produzido como entrega à
%  parte. Atualizar a referência acima com o nome do arquivo final.]
...
\begin{figure}[htbp]
    ...
    \node[plan, minimum width=22mm] (snd) at (17.75,1.25) {\textbf{Sound Generator}};
    \node[plan, minimum width=16mm] (aud) at (20.6,1.25) {\textbf{Áudio}};
    ...
    \draw[bus2] (snd.south) -- (snd  |- O) node[wlbl,right]{32\,b};

    % ===================== Cartridge / external (top-left) =====================
    \node[ext] (pakrom) at (2.4,3.2) {\textbf{SDRAM}\\PAK\_ROM\\(externa)};
    \node[ext] (sdramc) at (2.4,1.25) {\textbf{SDRAM}\\\textbf{Controller}};
    ...
    \draw[bus2] (sdramc.south) -- (sdramc |- O) node[wlbl,right]{16\,bit};
    ...
    \node[mem] (ewram) at (11.2,-2.8) {\textbf{EWRAM}\\256\,KiB};
    \node[mem] (vram)  at (13.4,-2.8) {\textbf{VRAM}\\96\,KiB};
    ...
    \foreach \n / \x in {bios/32,iwram/32,ewram/16,vram/16,io/16}{
    ...
    \caption{Arquitetura-objetivo do projeto. O núcleo ARM7TDMI (azul) já se encontra implementado e
        ...
        regiões internas (BIOS, EWRAM, IWRAM, VRAM, I/O, Palette, OAM),
        \texttt{PAK\_RAM} e o controlador da SDRAM externa, que abriga
        exclusivamente a \texttt{PAK\_ROM}, além da arbitragem de barramento e do DMA,
...
% ========== Referências ==========
% [INDICAR: caso a entrega final exija o estilo ABNT puro,
%  migrar este bloco para um arquivo .bib e ativar
%  \usepackage[num]{abntex2cite} +
%  \bibliographystyle{abntex2-num} +
%  \bibliography{...}. Por ora, mantemos o ambiente
%  \texttt{thebibliography} para garantir compilação
%  independente de pacotes externos.]
\begin{thebibliography}{99}

\bibitem{ARM_DDI0210B}
ARM LIMITED.
\textbf{ARM7TDMI Technical Reference Manual}.
Cambridge: ARM Limited, 2004.
Documento DDI~0210B.

\bibitem{ARM_DVI0027B}
ARM LIMITED.
\textbf{ARM7TDMI (Rev~3) Core Processor Product Overview}.
Cambridge: ARM Limited, 2001.
Documento DVI~0027B.

\bibitem{Copetti2019}
COPETTI, R.
\textbf{Game Boy Advance Architecture: a Practical Analysis}.
2019.
Disponível em:
...
\bibitem{Vijn_Tonc}
VIJN, J.
\textbf{Tonc, GBA Programming in Rot13}.
Disponível em: \url{https://gbadev.net/tonc/hardware.html}.
Acesso em: 15 abr.\ 2026.

\bibitem{GBATEK}
KORTH, M.
\textbf{GBATEK, GBA/NDS Technical Information}.
Disponível em: \url{https://problemkaputt.de/gbatek.htm}.
Acesso em: 15 abr.\ 2026.

\bibitem{PattersonHennessy}
PATTERSON, D.~A.; HENNESSY, J.~L.
\textbf{Computer Organization and Design: The Hardware/Software
Interface}.
6.~ed.\ Burlington: Morgan Kaufmann, 2020.

\bibitem{HarrisHarris}
HARRIS, D.~M.; HARRIS, S.~L.
\textbf{Digital Design and Computer Architecture, ARM Edition}.
Burlington: Morgan Kaufmann, 2015.

\bibitem{Trimberger2015}
TRIMBERGER, S.~M.
Three Ages of FPGAs: A Retrospective on the First Thirty Years
of FPGA Technology.
\textbf{Proceedings of the IEEE},
v.~103, n.~3, p.~318--331, 2015.

\bibitem{CycloneV}
INTEL CORPORATION (ALTERA).
\textbf{Cyclone V Device Handbook}.
San Jose: Intel Corporation, 2018.

\bibitem{TerasicDE1SoC}
TERASIC INC.
\textbf{DE1-SoC User Manual}.
Hsinchu: Terasic Inc., 2018.

\bibitem{MiSTer}
MiSTer FPGA PROJECT.
\textbf{MiSTer FPGA Project, Open-Source Hardware Reproduction
of Classic Consoles}.
Disponível em:
\url{https://github.com/MiSTer-devel/Main_MiSTer/wiki}.
Acesso em: 15 abr.\ 2026.

\bibitem{AnaloguePocket}
ANALOGUE INC.
\textbf{Analogue Pocket: Technical Specifications}.
Disponível em: \url{https://www.analogue.co/pocket}.
Acesso em: 15 abr.\ 2026.

\bibitem{NanoBoyAdvance}
SCHIBLI, F.
\textbf{NanoBoyAdvance: a Cycle-Accurate Game Boy Advance
Emulator}.
Disponível em:
...
\bibitem{mGBA}
ENDRIFT.
\textbf{mGBA: a Game Boy Advance Emulator}.
Disponível em: \url{https://mgba.io/}.
Acesso em: 15 abr.\ 2026.
