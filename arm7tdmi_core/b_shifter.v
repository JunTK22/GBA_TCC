...
assign special = ~shift_data[0] && (shift_data[7:3] == 5'b0);
    ...
    if (Imm_Operand_f) begin
        ...
        case (shift_data[2:1])
            ...
            2'b01: begin
                // Logical right
                intermediate  = intermediate2 >> shift;
                result[31:-1] = special ? {32'b0, intermediate[31]} : intermediate[31:-1];
            end
            2'b10: begin
                // Arithmetic right
                intermediate  = intermediate2 >>> shift;
                result[31:-1] = special ? {intermediate[31] ? {32{1'b1}} : 32'b0, intermediate[31]} : intermediate[31:-1];
            end
            2'b11: begin
                // Rotate right
                intermediate  = intermediate2 >> shift;
                result[31:-1] = special ? {carry_i, intermediate[31:0]} :  {intermediate[-1:-32] | intermediate[31:0], intermediate[-1]};
            end
        endcase
    end
end
