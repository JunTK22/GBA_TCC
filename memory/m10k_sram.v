...
module sram #(
    ...
    always @(*) begin
        if (!rst_n) begin
            ...
            if (!misalign_comb) begin // blocking: use value immediately below
                case (size)
                    // ----------------------------------------------------------
                    //  Byte read
                    // ----------------------------------------------------------
	...
	m10k m10k #(
		...
		.addr	(word_addr), // byte address 
		.wdata	(wdata_merged), // write data (right-aligned)
		.we		(we && !misalign_comb), // write enable
		
		.rdata	(raw_word) // read  data (right-aligned)
	);
